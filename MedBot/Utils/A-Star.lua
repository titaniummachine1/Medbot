...
function AStar.NormalPath(startNode, goalNode, nodes, adjacentFun)
    ...
    local iterations = 0
    local maxIterations = 100000 -- Safety limit
    ...
    while not openSet:empty() and iterations < maxIterations do
        ...
        -- Skip stale entries for nodes already processed
        ...
        if not success or type(neighbors) ~= "table" then
            -- If adjacency function fails or returns invalid data, skip this node
            goto continue
        end
        ...
        for _, neighborData in ipairs(neighbors) do
            ...
            if not gScore[neighbor] or tentativeGScore < gScore[neighbor] then
                ...
                fScore[neighbor] = tentativeGScore + heuristicCost(neighbor, goalNode)
