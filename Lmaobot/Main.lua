...
local Log = Common.Log.new("Lmaobot")
Log.Level = 0
    ...
    if G.pLocal.flags & FL_ONGROUND == 1 or G.pLocal.entity:EstimateAbsVelocity():Length() < 50 then
        ...
        if G.Navigation.currentNodeTicks > 264 or (G.Navigation.currentNodeTicks > 22
            ...
            if not Navigation.isWalkable(LocalOrigin, G.Navigation.currentNodePos, 1) then
                ...
                if G.Navigation.path[G.Navigation.currentNodeIndex] and G.Navigation.path[G.Navigation.currentNodeIndex + 1] then
                    Navigation.RemoveConnection(G.Navigation.path[G.Navigation.currentNodeIndex], G.Navigation.path[G.Navigation.currentNodeIndex + 1])
                elseif G.Navigation.path[G.Navigation.currentNodeIndex] and not G.Navigation.path[G.Navigation.currentNodeIndex + 1] and G.Navigation.currentNodeIndex > 1 then
                    Navigation.RemoveConnection(G.Navigation.path[G.Navigation.currentNodeIndex - 1], G.Navigation.path[G.Navigation.currentNodeIndex])
                end
                Navigation.ClearPath()
                Navigation.ResetTickTimer()
...
local function OnCreateMove(userCmd)
    ...
    if handleUserInput(userCmd) then return end --if user is walking
...
callbacks.Unregister("CreateMove", "Lmaobotv2.CreateMove")
callbacks.Unregister("DrawModel", "Lmaobotv2.DrawModel")
callbacks.Unregister("FireGameEvent", "Lmaobotv2.FireGameEvent")

callbacks.Register("CreateMove", "Lmaobotv2.CreateMove", OnCreateMove)
callbacks.Register("DrawModel", "Lmaobotv2.DrawModel", OnDrawModel)
callbacks.Register("FireGameEvent", "Lmaobotv2.FireGameEvent", OnGameEvent)
