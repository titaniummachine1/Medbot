...
local function OnDrawMenu()
    ...
    if Lbox_Menu_Open == true and ImMenu and ImMenu.Begin("Lmaobot V2", true) then
        ...
        if Tabs.Main then
            ImMenu.BeginFrame(1)
                G.Menu.Main.Enable = ImMenu.Checkbox("Enable Main Features", G.Menu.Main.Enable)
            ImMenu.EndFrame()
            ...
            ImMenu.BeginFrame(1)
                WorkManager.workLimit = ImMenu.Slider("Work Limit", WorkManager.workLimit, 1, 20)
            ImMenu.EndFrame()
            ...
            ImMenu.BeginFrame(1)
                G.Menu.Main.smoothFactor = ImMenu.Slider("Smooth Factor", G.Menu.Main.smoothFactor, 0.01, 0.1, 0.01)
            ImMenu.EndFrame()
        end
